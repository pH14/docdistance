...
class ReverseIndex(MapReduce):
    ...
    # Produce a (key, value) pair for each word and its byte offset
    def Map(self, keyvalue, value):
        results = []
        i = 0
        n = len(value)
    ...
    # Reduce to (key, [byte_offset_1, byte_offset_2...])
    def Reduce(self, key, keyvalues):
...
if __name__ == '__main__':
  ...
  wc = WordCount(16, 16, sys.argv[1])
  wc2 = WordCount(16, 16, sys.argv[2])
  #wc = ReverseIndex(8, 8, sys.argv[1])
  # Run it
  ...
  out1_mag = sum(y for (x, y) in out)
  out2_mag = sum(y for (x, y) in out2)

  print "magnitudes %s %s" % (out1_mag, out2_mag)
  print out
